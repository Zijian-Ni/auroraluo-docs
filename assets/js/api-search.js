(function(){
  ...
  function score(q, t){ if (!q) return 0; t=normalize(t); q=normalize(q);
    if (t===q) return 100; if (t.startsWith(q)) return 80; if (t.includes(q)) return 60;
    let hit=0; q.split(/[^a-z0-9_]+/).forEach(p=>{ if(p && t.includes(p)) hit+=10; }); return hit; }
  ...
  async function loadIndex(){ try{ const res=await fetch('api/index.json'); return await res.json(); }catch(e){ return []; } }
  async function init(){
    const data=await loadIndex(), input=document.getElementById('api-q'); if(!input) return;
    function run(){ const q=input.value.trim(); let out=[];
      data.forEach(item=>{
  ...
  if (window.$docsify){ window.$docsify.plugins=[].concat(function(hook){ hook.doneEach(function(){ if (location.hash.includes('api/Search')) setTimeout(init,80); }); }, window.$docsify.plugins||[]); }
